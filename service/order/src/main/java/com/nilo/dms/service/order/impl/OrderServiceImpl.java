...
public class OrderServiceImpl extends AbstractOrderOpt implements OrderService {
	...
	private void notifyStatusUpdate(OrderOptRequest optRequest, String orderNo, String referenceNo,
		...
		try {
			...
			MerchantConfig merchantConfig = JSON.parseObject(
					RedisUtil.get(Constant.MERCHANT_CONF + optRequest.getMerchantId()), MerchantConfig.class);
			InterfaceConfig interfaceConfig = JSON.parseObject(
					RedisUtil.hget(Constant.INTERFACE_CONF + optRequest.getMerchantId(), "update_status"),
	...
	private void routeRecord(OrderOptRequest optRequest, String orderNo) {
		...
		RouteConfig routeConfig = JSON.parseObject(
				RedisUtil.hget(Constant.ROUTE_CONF + optRequest.getMerchantId(), optRequest.getOptType().getCode()),
				RouteConfig.class);
	...
	private DeliveryOrder convert(DeliveryOrderDO d) {
		...
		if (d.getNetworkId() != null) {
			DistributionNetworkDO networkDO = JSON.parseObject(
					RedisUtil.hget(Constant.NETWORK_INFO + d.getMerchantId(), "" + d.getNetworkId()),
					DistributionNetworkDO.class);
			deliveryOrder.setNetworkDesc(networkDO.getName());
		}
		if (d.getNextNetworkId() != null) {
			DistributionNetworkDO networkDO = JSON.parseObject(
					RedisUtil.hget(Constant.NETWORK_INFO + d.getMerchantId(), "" + d.getNextNetworkId()),
					DistributionNetworkDO.class);
