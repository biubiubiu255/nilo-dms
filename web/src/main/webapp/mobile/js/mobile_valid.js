...
(function( $ )
{
	...
	if ( $.fn[ _PLUGIN_ ] )
	{
        ...
        function ValidVal(form, opts)
        {
			...
			$("#"+formId+ " .submit").unbind("click").bind("click",function(event) {									
				...
				if(auditAttr == '1'){
					if(opts.warningMsgType == 'tips'){
						warningTipMsg('已审核的记录不允许操作');
					}else{
						showWarning('已审核的记录不允许操作');
					}
					return false;
				}
                if (form_data) {
                    $.ajax({
                        ...
                        success: function(json) {
                            try {
                                ...
                                if (showMsg) {
                                    if (response.result) {
                                        var msg = (successMsg == '') ? response.msg : successMsg;
                                        //单独submit的form不需要hide
                                        if(action != 'submit'){
                                        	$('#'+formId).hide();
                                        }
                                        msgbox(msg);
										var primaryId = toObject(response.data);
                                        if(typeof(primaryId) == 'object'){
                                            primaryId = primaryId.id;    
                                        }
										if(action == 'new'){
											mbObject.appendNewRecord(primaryId);
										}else if(action == 'edit'){
											mbObject.updateModifyRecord(opts.editId);	
										}
                                    } else {
                                        warningTipMsg(response.msg);
                                    }
                                }
                                if (callback) {
	...
	ValidVal.prototype = {
		...
		validate: function( body, callCallback )
		{
                        		...
                        		vf = $f.data( _FIELD_ );
                                        ...
                                        if ( vf )
                                        {
                                                ...
                                                if ( vf.valid )
                                                {
                                                        ...
                                                        if ( $f.attr( 'name' ).indexOf( '_time' ) != -1 && v != '' && v.indexOf( 'T' ) != -1 ) {
                                                                v = v.replace( 'T', ' ' );
                                                        }
                                                        if ( $f.is( '[type="radio"]' ))
                                                        {
                                                        ...
                                                        if ( $f.is( '[type="checkbox"]' ) )
                                                        {
                                                                if ( $f[0].checked )
                                                        	{
                                                                        if (typeof data_obj[ $f.attr( 'name' )] == 'undefined' || data_obj[ $f.attr( 'name' )] == '')
                                                                        {
                                                                                data_obj[ $f.attr( 'name' ) ] = v;
                                                                        }
                                                                        else
                                                                        {
                                                                                data_obj[ $f.attr( 'name' ) ] = data_obj[ $f.attr( 'name' ) ] + ',' + v;
                                                                        }
                                                        	}
                                                                return;
                                                        }
                                                        
                                                        if ( $f.is( '[type="datetime-local"]' ) &&  v != '')
                                                        {
                                                        		v = v.replace("T"," ")+":00"; 
                                                    			data_obj[ $f.attr( 'name' ) ] = v;
                                                    			return;
                                                        ...
                                                        if ( typeof data_obj[ $f.attr( 'name' ) ] == 'undefined' )
                                                        {
                                                                data_obj[ $f.attr( 'name' ) ] = v;
                                                        }
                                                }
			...
			if ( miss_arr.length > 0 )
			{
				...
				{
					this.opts.form.onInvalid.call( this.form[ 0 ], miss_arr, this.opts.language ,this.opts.warningMsgType );
				}
				return false;
			}
		...
		loadForm: function(rowdata)
		{
			...
			this.opts.validate.fields.filter( $(_INPUTS_, this.form) ).each(
				function()
				{
					var $f = $(this),
						vf = $f.data( _FIELD_ );
					var field_name = $f.attr("name");
					if ( vf && rowdata[field_name] != 'undefined')
					{
						var field_value = rowdata[field_name];
						$f.val(field_value);
						vf.isValid( true, true );
					}
				}
	...
	ValidValField.prototype = {
		...
		_init: function()
		{
			...
			if ( this.field.is( 'input') && this.field.attr( 'maxlength' ) != 'undefined' )
			{
				var maxLength = parseInt(this.field.attr( 'maxlength'));
				this.field.bind(
					namespace( 'keyup' ),
					function( event )
					{
	...
	$.fn[ _PLUGIN_ ].defaults = {
		...
		'form'	: {
			...
			'onValidate': null,
			'onValid'	: null,
			'onInvalid'	: function( fieldArr, language ,warningMsgType)
			{
				switch ( language )
				{
					case 'nl':
